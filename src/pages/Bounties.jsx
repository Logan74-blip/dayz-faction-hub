...
export default function Bounties({ session }) {
  ...
  async function post() {
    ...
    const { data, error } = await supabase.from('bounties').insert({
      ...
      status: (role === 'leader' || role === 'co-leader') ? 'active' : 'pending'
  ...
  const statusColor = { pending:'var(--yellow)', active:'var(--red)', completed:'var(--muted)' }
    ...
    <div style={{ maxWidth:800, margin:'40px auto', padding:'0 24px', display:'flex', flexDirection:'column', gap:'24px' }}>
      ...
      {showForm && (
        <div className="card" style={{ display:'flex', flexDirection:'column', gap:'12px', borderColor:'var(--red)' }}>
          ...
          <input placeholder="Target player name..." value={form.target_name} onChange={e => setForm(f => ({...f, target_name:e.target.value}))} />
          <input placeholder="Reward (e.g. AK-74 + 2 mags)" value={form.reward} onChange={e => setForm(f => ({...f, reward:e.target.value}))} />
          ...
          <div style={{ display:'flex', gap:'8px' }}>
            <button className="btn btn-red" onClick={post}>Post Bounty</button>
          ...
          {active.map(b => (
            <div key={b.id} className="card" style={{ borderLeft:'3px solid var(--red)', display:'flex', gap:'12px', alignItems:'center' }}>
              <div style={{ flex:1 }}>
                ...
                <div style={{ fontSize:'11px', color:'var(--muted)', marginTop:'6px' }}>Posted by {b.profile?.discord_username} • {new Date(b.created_at).toLocaleDateString()}</div>
              </div>
              {canApprove && (
      ...
      {completed.length > 0 && (
        <div style={{ display:'flex', flexDirection:'column', gap:'8px' }}>
          ...
          {completed.map(b => (
            <div key={b.id} className="card" style={{ borderLeft:'3px solid var(--border)', opacity:0.6, display:'flex', justifyContent:'space-between', alignItems:'center' }}>
              <div>
