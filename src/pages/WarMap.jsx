...
export default function WarMap({ session }) {
  ...
  function onWheel(e) {
    ...
    setScale(s => Math.min(Math.max(s * delta, 0.3), 8))
  }

  function onMouseDown(e) {
    if (e.button !== 0) return
    setDragStart({ x: e.clientX - offset.x, y: e.clientY - offset.y, startX: e.clientX, startY: e.clientY })
  }
  ...
  function onMouseMove(e) {
    ...
    setOffset({ x: dx, y: dy })
    const moved = Math.abs(e.clientX - dragStart.startX) > 5 || Math.abs(e.clientY - dragStart.startY) > 5
  ...
  const currentImage = showTopo && selectedMap.topo ? selectedMap.topo : selectedMap.image
    ...
    <div style={{ display:'flex', flexDirection:'column', height:'calc(100vh - 56px)', background:'var(--bg)' }}>
      ...
      <div style={{
        ...
        {selectedMap.topo && (
          <button
            ...
            style={{
              ...
              border: showTopo ? '1px solid var(--green)' : '1px solid var(--border)',
              color: showTopo ? 'var(--green)' : 'var(--muted)'
            }}
          >
        ...
        <div style={{ marginLeft:'auto', display:'flex', gap:'6px', alignItems:'center' }}>
          <button onClick={() => setScale(s => Math.min(s * 1.2, 8))} className="btn btn-ghost" style={{ padding:'4px 10px', fontSize:'16px' }}>+</button>
          <span style={{ color:'var(--muted)', fontSize:'12px', minWidth:'40px', textAlign:'center' }}>{Math.round(scale * 100)}%</span>
          <button onClick={() => setScale(s => Math.max(s * 0.8, 0.3))} className="btn btn-ghost" style={{ padding:'4px 10px', fontSize:'16px' }}>−</button>
        ...
        {faction && <span style={{ color:'var(--muted)', fontSize:'12px' }}>Click map to place · Scroll to zoom · Drag to pan</span>}
      </div>
      ...
      >
        <div style={{
          ...
          <div ref={mapRef} style={{ position:'relative', display:'inline-block' }}>
            ...
            {/* Loading overlay */}
            {currentImage && !mapLoaded && !mapError && (
              <div style={{
                ...
                <span style={{ fontFamily:'Share Tech Mono', color:'var(--green)', fontSize:'14px' }}>
                  LOADING {selectedMap.name.toUpperCase()}...
                </span>
              </div>
            )}
            ...
            {pending && (
              <div style={{
                position:'absolute', left:`${pending.x}%`, top:`${pending.y}%`,
            ...
            {filteredMarkers.map(m => {
              ...
              return (
                <div
                  key={m.id}
                  style={{
                    position:'absolute', left:`${m.lng}%`, top:`${m.lat}%`,
        ...
        <div style={{
          ...
          {filteredMarkers.length} MARKERS · {selectedMap.name.toUpperCase()} · {showTopo ? 'TOPO' : 'SATELLITE'}
        </div>
      </div>
      ...
      {pending && (
        <div style={{
          position:'fixed', top:'80px', right:'20px', zIndex:1000, width:'280px'
        }} className="card">
