...
export default function Resources({ session }) {
  ...
  async function importOcrItems() {
    ...
    const toImport = selectedOcr.map(i => ({
      faction_id: faction.id, created_by: userId,
      name: ocrResults[i].name, category: ocrResults[i].category,
      quantity: ocrResults[i].quantity, notes: ocrResults[i].possibleMod ? 'Possible mod item' : ''
    }))
    const { data, error } = await supabase.from('resources').insert(toImport).select()
    if (!error) { setItems(p => [...p, ...data]); setOcrMode(false); setOcrImage(null); setOcrResults([]); setSelectedOcr([]) }
  }
    ...
    <div style={{ maxWidth:900, margin:'40px auto', padding:'0 24px', display:'flex', flexDirection:'column', gap:'24px' }}>
      ...
      {ocrMode && (
        <div className="card" style={{ display:'flex', flexDirection:'column', gap:'14px', borderColor:'var(--green-dim)' }}>
          ...
          <p style={{ fontSize:'13px', color:'var(--muted)' }}>Upload a screenshot of your inventory or stash and we'll detect the items automatically.</p>
          {!ocrImage && (
            <>
          ...
          {ocrResults.length > 0 && (
            <>
              <p style={{ fontSize:'13px', color:'var(--muted)' }}>Found <strong style={{ color:'var(--green)' }}>{ocrResults.length} items</strong>. Select which to import:</p>
              <div style={{ display:'flex', flexDirection:'column', gap:'6px', maxHeight:'300px', overflowY:'auto' }}>
                {ocrResults.map((item, i) => (
                  <div key={i} onClick={() => toggleOcrSelect(i)} style={{ display:'flex', alignItems:'center', gap:'10px', padding:'8px 12px', background: selectedOcr.includes(i) ? '#0d1a0d' : 'var(--surface)', borderRadius:'6px', border:`1px solid ${item.possibleMod ? 'var(--yellow)' : selectedOcr.includes(i) ? 'var(--green-dim)' : 'var(--border)'}`, cursor:'pointer', opacity: selectedOcr.includes(i) ? 1 : 0.5 }}>
                    <input type="checkbox" checked={selectedOcr.includes(i)} onChange={() => toggleOcrSelect(i)} onClick={e => e.stopPropagation()} style={{ width:'auto', accentColor:'var(--green)' }} />
                    <div style={{ flex:1 }}>
                    ...
                    <input type="number" value={item.quantity} min={1} onChange={e => setOcrResults(r => r.map((x, idx) => idx === i ? {...x, quantity: parseInt(e.target.value) || 1} : x))} onClick={e => e.stopPropagation()} style={{ width:'60px', textAlign:'center', fontSize:'13px' }} />
                  </div>
                ))}
              </div>
      ...
      {faction && (
        <div className="card" style={{ display:'flex', flexDirection:'column', gap:'12px' }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 80px', gap:'10px' }}>
            <input placeholder="Item name (e.g. AK-74)" value={form.name} onChange={e => setForm(f => ({...f, name:e.target.value}))} onKeyDown={e => e.key === 'Enter' && addItem()} />
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:'8px' }}>
        ...
        {filtered.map(item => (
          <div key={item.id} className="card" style={{ display:'flex', alignItems:'center', gap:'16px', padding:'14px 20px' }}>
            ...
            <button onClick={() => deleteItem(item.id)} className="btn btn-ghost" style={{ padding:'6px', border:'none' }}><Trash2 size={15} color="var(--red)" /></button>
          </div>
        ))}
      </div>
    </div>
