...
import { Pin, Edit2, Trash2, X, Check, Heart } from 'lucide-react'
...
export default function HubAnnouncements({ session }) {
  ...
  const [isOwner, setIsOwner]             = useState(false) // ✅ comes from Supabase now
  ...
  // ─── Check if current user is a server admin (god mode) ────────────────────
  // Your ID never appears in frontend code — it's checked securely in Supabase
  useEffect(() => {
    async function checkAdmin() {
      ...
      if (!error && data) setIsOwner(true)
    }
    checkAdmin()
  }, [userId])

  // ─── Load announcements + likes ────────────────────────────────────────────
  useEffect(() => {
    loadAll()
  }, [])
  ...
  async function toggleLike(id) {
    if (myLikes.has(id)) {
      await supabase.from('hub_announcement_likes')
        .delete()
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '8px', paddingTop: '8px', borderTop: '1px solid var(--border)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                {post.profile?.discord_avatar && (
                  <img
                    src={post.profile.discord_avatar}
